...
import { shorten, formatter_commas, debounce } from "../../helpers/utils";
...
const App = (props) => {
  ...
  let chart = (
    <Newchartmemo data={chart_data} height={activeIndex} type="column" />
  );
...
const Testchart = (props) => {
  ...
  var chart;
  if (type == "column") {
    chart = <AntChart.Column {...config} />;
  } else if (type == "line") {
    chart = <AntChart.Line {...config} style={{}} />;
  } else if (type == "pie") {
  ...
  return (
    ...
    <div style={{ height: props.height ?? 400 }}>{chart}</div> //</div>
  );
};
