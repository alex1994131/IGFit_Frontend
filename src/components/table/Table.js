...
async function getTotal() {
  ...
  var options = {
    headers: {
      "x-developer-key":
        "981081937f8f6098257c0e2619de68639a87852eb8f51d5db1e8dd00a3c24e3e342ee7902c2a0f0dff518de06c4c6ab3e10a62ec5bc761a1643cc027952da3f0",
  ...
  const res = reducearray(
    tx,
    (txdata) => new Date(txdata["date"]).getTime(),
    (txdata) => txdata["amount"],
    (acc, value) => acc + value
  );
...
function reducearray(tx, getkey, getvalue, sumvalue) {
  ...
  var tx_new = tx.map((txdata) => {
    return {
      key: getkey(txdata),
      value: { total: getvalue(txdata), data: [txdata] },
    };
  });

  const res = Array.from(
    tx_new.reduce((m, { key, value }) => {
      return m.set(key, {
        total: sumvalue(m.get(key)?.total || 0, value.total),
        data: m.get(key) ? [...m.get(key).data, value.data[0]] : value.data,
      });
    }, new Map()),
    ([key, value]) => ({ key, value })
  );
...
const App = () => {
  ...
  var options = {
    ...
    onTableInit: (action, tableState) => {
      ...
      const res = reducearray(
        search_data,
        (txdata) => txdata.x.getTime(),
        (txdata) => txdata.y,
        (acc, value) => acc + value
      );
      const tabledata = res.map((element) => {
        return { x: element.key, y: element.value.total };
      });
  ...
  if (fetch) {
    ...
    var config = {
      ...
      tooltip: {
        fields: ["y"],
        formatter: (datum) => {
          return { name: config.yField, value: datum.y.toFixed(1) };
        },
      },
    };
