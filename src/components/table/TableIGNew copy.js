...
import ChartEditDataComponent from "../components/chart/ChartEditDataComponent.js";
    ...
    if (props.acc?.positions) {
      ...
      for (let [name, value] of Object.entries(props.acc.positions)) {
        if (value?.price_chart) {
          let end_date =
            value.price_chart[value.price_chart.length - 1].compare_date;
          for (let [day, index] of Object.entries(price_index)) {
            ...
            let start_price = value.price_chart.find(
              (e) => e.compare_date == compare_date.getTime()
            );
            //if (start_price) {
            index.push({
              x: value.ticker,
              y: start_price
                ? (value.price_chart[value.price_chart.length - 1].value.close /
                    start_price.value.close) *
                  100
                : null,
            });
            //}
          ...
          <Grid.Column width={9}>
            <div>
              <TableIG3
                ...
                height={Responsive(breakpoints, 550, 900)}
              />
            </div>
    ...
    if (priceIndex[days]?.length) {
      return (
        <Newchartmemo
          ...
          setcolor={(e) => {
            return priceIndex[days].find((e2) => e2.x == e.x).y > 100
              ? null
              : "grey";
          }}
          Axis={{ nice: true, line: null, label: null, grid: null }}
