...
import {
  getPocketsmithStatus,
  updatePocketsmithData,
  updatePocketsmithTransactions,
  fetchAllTransactions,
} from "../../helpers/Pocketsmith";
...
const App = (props) => {
  ...
  const renderHeader = (sortDir, setSortDir, filteredData) => {
    return () => (
      ...
      >
        <span>
          Sort by Date <Icon name={"arrow " + (sortDir ? "down" : "up")} />
        </span>
        {!props.minimal ? (
          <span style={{ textAlign: "right" }}>
            Transactions: {formatter_commas(filteredData.length)} | Income:{" "}
            {formatter_commas(
              sumif(filteredData, (e) => {
                if (
                  e["amount_in_base_currency"] >= 0 &&
                  e["is_transfer"] != true
                ) {
        ...
        <Modal.Content image>
          ...
          <Modal.Description style={{ width: "100%" }}>
            <Header style={{ margin: "8px 0" }}>
              {selectedrow.current?.date}
            </Header>

            <div style={{ color: "#888", fontSize: 14, margin: "8px 0" }}>
              {selectedrow.current?.["transaction_account.name"]}
            </div>
            ...
            >
              <div style={{ width: "70%" }}>{selectedrow.current?.payee}</div>
              ...
              >
                {Number(
                  selectedrow?.current?.["amount_in_base_currency"]
            ...
            <span style={{ marginRight: 20 }}>
              <Dropdown
                ...
                updateValue={(value) =>
                  updateValue(selectedrow.current, value, "category.title")
                }
                dropdown_list={categories.current}
                update={async (value, row) => {
                  return await updatePocketsmithData(
            ...
            <span style={{ position: "relative", top: -3 }}>
              <Dropdown
                ...
                row={selectedrow.current}
                updateValue={(value) =>
                  updateValue(selectedrow.current, value, "is_transfer")
                }
  ...
  const row = (rowData, sectionID, rowID) => {
    ...
    return (
      ...
      >
        ...
        >
          {obj["transaction_account.name"].toUpperCase()}
        </div>
        <div
          style={{ display: "-webkit-box", display: "flex", padding: "3px 0" }}
        >
          <div style={{ lineHeight: 1, width: "100%" }}>
            ...
            <div>
              ...
              <span style={{ fontSize: "12px", color: "#FF6E27" }}>
                {obj["category.title"]}
              </span>
              <span
                style={{
                  fontSize: "12px",
                  float: "right",
                  color: obj["amount_in_base_currency"] > 0 ? "green" : "red",
                }}
              >
                {Number(obj["amount_in_base_currency"]).toLocaleString(
                  undefined,
                  {
                    style: "currency",
                    currency: "GBP",
