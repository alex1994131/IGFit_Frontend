...
const App = (props) => {
  ...
  var handleRadioChange = useCallback(
    (e) => {
  ...
  useEffect(() => {
    if (props.data?.length > 0 && searchQuery) {
      ...
      let columns = Object.keys(props.data[0]).map((element) => {
        return { name: element, display: "true" };
      });
      let { rows_matched, query } = findmatchintable(
        searchQuery,
        props.data,
        columns,
  ...
  useEffect(() => {
    ...
    if (acc_result) {
        ...
        for (let date_key of dates) {
          ...
          for (let [key, value] of Object.entries(obj)) {
            let data = {
              ...
              category: key,
              rows: acc.result_rows[date_key][key],
            };
            chart_data.push(data);
            categories_total[key] =
              (categories_total[key] || 0) + value * acc.multiply_sign;
          }
      ...
      let category_arr = Object.entries(categories_total)
        .filter((e) => Math.abs(e[1]) > category_limit)
        .sort((a, b) => b[1] - a[1]);
      ...
      function categorySelectFn() {
        ...
        return (event) => {
          ...
          if (
            event.data.data.x == selectedDate &&
            event.data.data.category == selectedCategory &&
            showTable.current
          ) {
            showTable.current = 0;
            setShow(0);
      ...
      for (let [category, value] of category_arr) {
        ...
        for (let i = min_date; i <= end_date.getTime(); ) {
          ...
          next_month.setUTCMonth(
            next_month.getUTCMonth() + (acc_result[0].type == "m" ? 1 : 12)
          );
        ...
        let yAxis = scaleButton
          ? {
              min: 0,
              max:
                acc_result[0].type == "m"
                  ? 2000
                  : 20000 /* tickInterval: 200 */,
            }
          : { min: undefined, max: undefined, tickInterval: undefined };
        ...
        chart_titles.push({
          category: category,
          title:
            shorten(category_name, 20) +
            ": £" +
            formatter_commas(value.toFixed(0)) +
            "",
          name: shorten(category_name, 20),
        });

        let chart = (
          <Grid.Column key={"NewChartPx1" + category}>
            {/*<div style={{ fontWeight: 'normal', fontSize:"14px", margin:"10px 0 1px 0", textAlign:"left", textOverflow:"ellipsis", whiteSpace:"nowrap", overflow:"auto" }}>{shorten(category_name,20)+": £"+formatter_commas(value.toFixed(0))+""}</div> */}
            <Newchartmemo
              data={category_data}
              type="column"
              height={120}
        ...
        <Grid.Row>
          ...
          <Grid.Column width={6}>
            <h3 style={{ margin: "0 0 5px 0" }}>
              {!showTable.current
                ? "Select a Category"
                : titles.find((e) => e.category == selectedResult.category)
                    .name +
                  " (" +
                  formatDate(new Date(selectedResult.x)) +
                  ")"}
            </h3>
            <div style={{ display: showTable.current ? "inline" : "none" }}>
              ...
              {categoryData ? (
                <TablePx4
                  key="NewTableCategories2" /* set_search_data={cb2} */
                  data={categoryData}
                  minimal
                />
              ) : (
