...
import ChartEditDataComponent from "../chart/ChartEditDataComponent.js";
import NewTable from "./NewTable.js";
import TablePx4 from "./TablePx4.js";

import { dateStr, sumif } from "../../helpers/utils";
import { findmatchintable } from "../../helpers/filtermatches.js";
import {
  shorten,
  formatter_commas,
  debounce,
...
const App = (props) => {
  ...
  var handleInput = useCallback((e) => {
    ...
    let new_element = (
      <Grid.Column key={"gridchartdata " + saved_elements.length}>
        <ChartEditDataComponent
          id={"chartdata " + saved_elements.length}
          searchQuery="acc:category.title:m"
          data={props.data}
          style={{ margin: "50px 50px 50px 50px" }}
          title={
            <h4 style={{ margin: "0 0 5px 0" }}>
              {"Chart " + (saved_elements.length + 1)}
            </h4>
          }
          height={100}
        />
  ...
  const ytdcalc = {
    title: "YTD Analysis",
    searchQuery:
      "acc:amount:d category.title!=transfer category.title!=sal category.title!=inc category.title!=“credit card”",
  ...
  const mthcalc = {
    ...
    searchQuery:
      "acc:amount:m category.title!=transfer category.title!=sal category.title!=inc category.title!=“credit card”",
  ...
  const catcalc = {
    ...
    searchQuery:
      "acc:amount:d:8 category.title!=transfer category.title!=sal category.title!=inc category.title!=“credit card”" +
      (currentRange
        ? " acc:category.title AND (date>" +
          dateStr(currentRange[0]) +
          " AND date<" +
          dateStr(currentRange[1]) +
          ")"
        : "acc:category.title:m:1"),
    customRender: MonthAnalysis(
      currentRange,
      setCategoryData,
      (e) => {
        showTable.current = e;
  ...
  var resp = (
    <div /* style={{width:"90%", margin:"auto"}} */>
          ...
          <Grid.Row stretched>
            <Grid.Column width={6}>
              ...
              {Element(
                ...
                "Monthly Expense",
                props,
                Responsive(breakpoints, 125, 200),
                { legend: false }
              )}
            ...
            <Grid.Column width={10}>
              {Element(
                "acc:category.title:-m:3 category.title!=transfer category.title!=sal category.title!=inc category.title!=“credit card”",
                "3 Months View",
                props,
                Responsive(breakpoints, 350, 300),
                {
                  label: {
                    position: "middle",
                    style: {
                    ...
                    formatter: (...args) => {
                      if (
                        args[0].y /
                          Math.max(...Object.values(args[0].x_total)) >
                        0.1
                      )
                  ...
                  {categoryData?.length ? (
                    ...
                    >
                      <div
                        ...
                        style={{ height: 7, width: 7, marginRight: 7 }}
                      />
                      {categoryData?.length
                        ? categoryData[0]["category.title"] +
                          " " +
                          "(" +
                          (
                            (sumif(
                              categoryData,
                              (e) => e["amount_in_base_currency"]
                            ) /
                              sumdata.current) *
                            100
                          ).toFixed(0) +
      ...
      <div style={{ display: showTable.current ? "inline" : "none" }}>
        {categoryData ? (
          <TablePx4 key="NewTableCategories" data={categoryData} minimal />
        ) : (
          ""
        )}
      </div>
    ...
    for (let acc of acc_result) {
      ...
      let dd =
        (new Date(
          ...
          first_date.getUTCDate()
        ) -
          new Date(new Date(dates[0]).getUTCFullYear(), 0, 1)) /
          1000 /
          60 /
          60 /
          24 +
      ...
      for (
        let day = first_date;
        day.getTime() <= new Date(dates[dates.length - 1]).getTime();
        ...
        if (
          day.getUTCDate() == 1 &&
          day.getUTCMonth() == (type == "y" ? 0 : ytd.getUTCMonth())
        ) {
          prev = 0;
          dd = 1;
        }
        if (
          (type == "y" && day.getUTCMonth() < ytd.getUTCMonth()) ||
          (day.getUTCMonth() == ytd.getUTCMonth() &&
            day.getUTCDate() <= ytd.getUTCDate())
        ) {
          ...
          let data = {
            ...
            y:
              prev +
              (result[date_key] ? Object.values(result[date_key])[0] : 0),
            category: day.getUTCFullYear().toString(),
          };
    ...
    for (let acc of acc_result) {
      ...
      for (let date_key of dates) {
        ...
        for (let [key, value] of Object.entries(obj)) {
          let data = {
            x: monthNames[new Date(date_key).getUTCMonth()],
            month: new Date(date_key).getUTCMonth(),
            y: value * acc.multiply_sign,
            category: new Date(date_key).getUTCFullYear().toString(),
          };
          chart_data.push(data);
        }
      }
      ...
      resp.push({
        ...
        chartProps: {
          type: "line",
          smooth: true,
          xAxis: { type: "cat", label: undefined },
          legend: {
            layout: "vertical",
            position: "right",
            animate: false,
...
const MonthAnalysis = (
  ...
  return (acc_result) => {
          ...
          chart_data.push(
            ...Object.keys(obj).map((e) => {
              ...
              return {
                x: e,
                y: Math.abs(obj[e]),
                rows: acc.result_rows[date_key][e],
              };
            })
          );
        } else if (acc.type == "d") {
    ...
    const text = !currentRange
      ? [
          ["MTD: ", sum],
          ["e(7d): ", (sum_7 / 7) * month_days],
          ["e(mtd): ", (sum / mtd) * month_days],
        ]
      : [["Total: ", sum]];
    sumdata.current = sum;
    ...
    resp.push({
      ...
      chartProps: {
            ...
            customHtml: () => {
              return (
                ...
                >
                  ...
                  {text.map((e, i) => (
                    <p key={"key " + i} style={{ margin: "0 0 0 0" }}>
                      <span
                        style={
                          text.length > 1
                            ? {
                      ...
                      {formatter_commas(e[1].toFixed(0))}
                    </p>
                  ))}
                  {/* /*  </li>
                        </ul> */}
        ...
        interactions: [
          {
            ...
            cfg: {
              start: [
                {
                  trigger: "element:touchend",
                  action: "element-single-highlight:toggle",
                  callback: cb,
                },
                {
        ...
        binding: [
          { event: "element:click", cb: bindevent },
          { event: "element:touchend", cb: bindevent },
        ],
        chartRef: chartRef,
      },
    });
...
function NewElement(element, props, height, children) {
  ...
  return (
    <ChartEditDataComponent
      id={"chartdata" + element.title}
      searchQuery={element.searchQuery}
      data={props.data}
      customRender={element.customRender}
      style={{ margin: "10px 10px 10px 10px" }}
      title={
        <>
          <h4 style={{ margin: "0 0 5px 0" }}>{element.title}</h4>
          {children}
        </>
      }
      height={height}
...
function Element(searchQuery, title, props, height, chartProps, getAnnotation) {
  return (
    <ChartEditDataComponent
      ...
      data={props.data}
      style={{ margin: "10px 10px 10px 10px" }}
      title={<h4 style={{ margin: "0 0 5px 0" }}>{title}</h4>}
      height={height}
      hidden
