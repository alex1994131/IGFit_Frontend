...
import { findmatchintable } from "../../helpers/filtermatches";
...
const App = (props) => {
  ...
  const genData = (tx, pIndex = 0) => {
    if (props.genData) {
      ...
      return genData2(
        props.getSectionID ? props.getSectionID : (e) => e.date,
        props.getRowID ? props.getRowID : (e) => e.id
      )(tx, pIndex);
    }
  };
  ...
  useEffect(() => {
    ...
    let data = filteredData.sort(
      (a, b) => props.sort(a, b) * (sortDir ? 1 : -1)
    );
    genData(data);
    let newData = dataSource.cloneWithRowsAndSections(
      listData.current.dataBlobs,
      listData.current.sectionIDs,
      listData.current.rowIDs
    );
  ...
  useEffect(() => {
    if (props.data?.length > 0 && searchQuery) {
      ...
      let { rows_matched2, query } = findmatchintable(
        searchQuery,
        props.data,
        columns,
        props.col_amount,
        ...
        if (!props.customRender) {
          for (let acc of acc_result) {
            ...
            if (type == "total") {
              ...
              let chart = (
                <Newchartmemo
                  key={"NewChartAccResult" + acc.name + acc.type}
                  data={chart_data}
                  type="pie"
                  height={props.height}
                  {...props.chartProps}
              ...
              if (group && dates.length > 0) {
                if (group == 1) {
                  category_arr = Object.entries(categories_total)
                    .sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]))
                    .slice(0, num_to_show);
                } else if (group == 2) {
                  category_arr = Object.entries(result[dates[dates.length - 1]])
                    .sort((a, b) => Math.abs(b?.[1]) - Math.abs(a?.[1]))
                    .slice(0, num_to_show);
                }
              }
              ...
              for (let date_key of dates) {
                ...
                for (let [key, value] of Object.entries(obj)) {
                  let data = {
                    ...
                    category: key,
                    x_total: date_total,
                    category_total: categories_total,
                  };
                  if (!group) {
              ...
              if (dates.length > 0) {
                if (group != 2) {
                  chart_data.sort(
                    (a, b) =>
                      Math.abs(categories_total[a.category]) -
                      Math.abs(categories_total[b.category])
                  );
                } else if (group == 2) {
                  chart_data.sort(
                    (a, b) =>
                      Math.abs(result[dates[dates.length - 1]][a.category]) -
                      Math.abs(result[dates[dates.length - 1]][b.category])
                  );
                }
  ...
  const onEndReached = (event) => {
    ...
    if (
      isLoading /* && !hasMore */ ||
      listData.current.txIndex == filteredData.length
    ) {
      return;
    }
    setLoading(1);
  ...
  return (
    <div>
      ...
      <div style={{ margin: "0 0 10px 0" }}>{calcCharts ? calcCharts : ""}</div>
      ...
      <ListView
        ...
        renderSectionHeader={(sectionData) => (
          <Sticky>{props.renderSectionHeader(sectionData)}</Sticky>
        )}
