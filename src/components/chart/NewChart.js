...
const NewChart = (props) => {
  ...
  var config = {
        ...
        content: (...args) => {
          if (args[0].percent > 0.01)
            return (
              args[0].x +
              "\n" +
              formatter_commas(args[0].y.toFixed(0)) +
              " / " +
              (args[0].percent * 100).toFixed(0) +
              "%"
            );
        },
      },
    ...
    tooltip: {
      ...
      formatter: (datum) => {
        return {
          name: type != "pie" ? datum.category : datum.x,
  ...
  if (props.yAxis) {
    config.yAxis = {
      /* min: props.yAxis.min, max: props.yAxis.max,  */ ...config.yAxis,
      ...props.yAxis,
    };
  ...
  if (props.xAxis) {
    ...
    if (props.xAxis.label && typeof props.xAxis.label == "object") {
      ...
      config.label = {
        ...config.label,
        offset: isXs || isSs ? 5 : 5,
        /* autoRotate:false, */ style: {
          ...config.label?.style,
          ...((isXs || isSs) && { fontSize: 10 }),
        },
      };
    }
  }
  ...
  if (props.annotations) {
    ...
    config.appendPadding = [
      0,
      (props.appendPaddingRight ?? 30) / divider,
      0,
      0,
    ];
  }
  ...
  if (props.slider) {
    ...
    if (ref.current) {
      let slider = ref.current.chart.controllers.find(
        (e) => e.name == "slider"
      );
      if (slider) {
        if (slider.slider) {
          ...
          let new_start = isNaN(found1 / (config.data.length - 1))
            ? 0
            : found1 / (config.data.length - 1);
          let new_end =
            isNaN(found2 / (config.data.length - 1)) || found2 == 0
              ? 1
              : found2 / (config.data.length - 1);
  ...
  if (type == "column") {
    chart = (
      <AntChart.Column
        {...config}
        /* style={{}} */ style={{ height: props.height ?? 400 }}
        chartRef={ref}
      />
    );
  } else if (type == "line") {
  ...
  return (
    <div>
      ...
      <div style={{ height: props.height ?? 400 }}>{chart}</div>
    </div>
  );
};
