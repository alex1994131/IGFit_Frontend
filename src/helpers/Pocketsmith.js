import { txdata2, txdata1 } from "../data/txdata";
...
export async function getTotal() {
  ...
  var header = {
    headers: { "x-developer-key": X_DEVELOPER_KEY, accept: "application/json" },
  };
  var response = await fetch(url, header).catch(() =>
    console.log("didn't fetch tx")
  );
...
export async function getTotal2() {
  ...
  var header = {
    headers: {
      "x-developer-key": X_DEVELOPER_KEY,
      accept: "application/json",
      Authorization: "Basic " + btoa("admin:secure_password"),
    },
  };
  var response = await fetch(url).catch(() =>
    console.log("didn't fetch total")
  );
...
export async function fetchAllTransactions(offlineMode = 0) {
  ...
  for (var i = 0; i < pages; i++) {
    //var url = "https://api.pocketsmith.com/v2/users/162199/transactions?per_page=100&page=" + (i + 1);
    var url =
      "https://faasd.tyap.cloud/function/pocketsmithtx2?per_page=100&page=" +
      (i + 1);
    tx_response.push(
      fetch(url, header).catch(() => console.log("didn't fetch tx"))
    );
  }

  await Promise.all(tx_response)
    .then((response) => {
...
export async function updatePocketsmithData(id, column, value) {
  ...
  var resp = await fetch(
    "https://faasd.tyap.cloud/function/pocketsmithtx3?id=" +
      id +
      "&column=" +
      column +
      "&value=" +
...
export async function getPocketsmithStatus() {
  ...
  try {
    resp = await fetch(
      "https://faasd.tyap.cloud/function/pocketsmithtx?lastupdate",
      header
    );
    if (resp) {
...
export async function updatePocketsmithTransactions() {
  ...
  try {
    resp = await fetch(
      "https://faasd.tyap.cloud/function/pocketsmithtx?updatetransactions",
      header
    );
  } catch (err) {
    console.log(err);
...
export const TxAccount = class TxAccount {
  ...
  constructor(full_tx, name, name_col = TRANSACTION_ACCOUNT_NAME_COLUMN) {
    ...
    this.tx.sort(
      (a, b) =>
        new Date(a[DATE_COLUMN]).getTime() - new Date(b[DATE_COLUMN]).getTime()
    );
    this.balances = {};
  }
  ...
  balanceToDate(
    date,
    [previousIndex = 0, previousBalance = 0],
...
export const Px = class Px {
  ...
  constructor(tx) {
    ...
    let dates = tx
      .map((e) => {
        return {
          date: e[DATE_COLUMN],
          dateobj: new Date(e[DATE_COLUMN]),
          date_timestamp: new Date(e[DATE_COLUMN]).getTime(),
        };
      })
    ...
    for (; date.getTime() <= end_date.date_timestamp; ) {
      ...
      for (let [name, account] of Object.entries(this.accounts)) {
        let balance;
        [balance, previousIndex[name]] = account.balanceToDate(
          date_obj,
          previousIndex[name]
        );
        account_balances.push(new TxAccountBalance(date_obj, account, balance));
      }
      let balance_history_added = new PxBalanceHistory(
        date_str,
        account_balances
      );
      ...
      this.account_total = {
        ...this.account_total,
        [date_str]: balance_history_added.total_balance_in_base_currency,
      };
  ...
  getBalancesAsData() {
    ...
    let resp = Object.entries(this.accounts).map(([a, b]) => {
      return { name: a, ...b.balances };
    });
