...
export function findmatches3(
  ...
  var result = false;
  if (
    saved_query.parsedQuery.length == 1 &&
    saved_query.parsedQuery[0].length == 0
  ) {
    result = true;
  } else {
  ...
  if (result) {
    ...
    accumulateRow(
      saved_query.acc_result,
      row,
      saved_query.date_col,
      saved_query.amount_col,
      row2
    );
  }
...
export function getQueryAndAccFromSearch(searchQuery) {
  ...
  try {
    ...
    accumulators = [];
    parsedQuery = parsedQuery.map((e) =>
      e
        .map((element) => {
          let arr = element.matchAll(
            /[a-zA-Z0-9_,.!=-]+["“”'‘’].*?["“”'‘’]|[a-zA-Z0-9_,.!:=><-]+|["“”'‘’].*?["“”'‘’]/g
          );
          arr = Array.from(arr).map((element) =>
            element[0]
              .trim()
              .replace(/["“”'‘’]/g, "")
              .toLowerCase()
...
export function findmatchintable(
  searchQuery,
  rows,
  columns,
  col_amount,
...
export function getAccumulators(accumulators, columns) {
  ...
  let acc = accumulators
    .map((e) => {
      ...
      if (
        columnnames.indexOf(elements[1]) > -1 &&
        !acc_result.find((e) => e.name == elements[1] && e.type == sum_type)
      ) {
        ...
        acc_result.push({
          ...
          result_rows: {},
          add: add,
          col: columnnames.indexOf(elements[1]),
        });
      }
      return {
        name: elements[1],
            ...
            if (equalto || notequalto) {
              if (columns[i2]?.name == category_filter[0]) {
                if (row[i2] && typeof row[i2] == "string") {
                  if (equalto) {
                    if (
                      row[i2]?.substr(0, category_filter[1].length) ==
                      category_filter[1]
                    ) {
                      isFound++;
                      break;
